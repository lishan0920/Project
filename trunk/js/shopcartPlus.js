...
$(function(){
   ...
   1.给表头中的全选框  绑定点击事件 点击的时候  获取到它的选中状态（true/false)
   2. 给表头中每一行的数据的input选择框，赋值为表头的选中状态（true/false)
  ...
  $theadInput.change(function(){
   ...
   $totalPriceInput.change(function(){
     ...
     $tbodyInputs.prop('checked',checkState);//赋值给表格中的全选框
  ...
  $tbodyInputs.change(function(){//给表格中的单选框绑定事件
    ...
    if(checkState === false){//如果有一个为false
...
$('.add').click(function(){//给增加绑定事件
  var count = parseInt($(this).next().val());//取后面输入框的值
   count++;//自增
   $(this).next().val(count);//把自增的值赋值给后面输入框
...
1.定义一个变量用于保存总价，用一个变量 用于保存已选商品数量
